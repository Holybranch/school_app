        ...
        {showAdminMenu && (
          <section className="bg-indigo-50 p-6 rounded-[2.5rem] border-2 border-indigo-100 animate-in slide-in-from-top-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleBulkCheckOut} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2">
        ...
        <section className="bg-white p-8 rounded-[3rem] shadow-2xl shadow-indigo-100/50 border border-white relative overflow-hidden">
          <form onSubmit={handleSearch} className="relative z-10">
            ...
            <div className="relative">
              <input
                type="text" inputMode="numeric" placeholder="학번 4자리 입력"
              ...
              />
              <button type="submit" className="absolute right-2 top-2 bottom-2 px-8 bg-indigo-600 text-white rounded-2xl font-black shadow-lg">확인</button>
            </div>
          </form>
          ...
          {foundStudent && (
            <div className="mt-8 p-8 bg-indigo-600 rounded-[2.5rem] text-white shadow-2xl animate-in zoom-in-95 duration-300">
              {foundStudent === 'not_found' ? (
                <div className="text-center py-4"><p className="text-xl font-black">등록되지 않은 학번입니다.</p></div>
              ) : (
                <div className="flex flex-col gap-6">
                  <div className="flex items-end justify-between">
                    ...
                    <div className="p-4 bg-white/10 rounded-3xl backdrop-blur-sm"><UserCheck size={36} className="text-white" /></div>
                  </div>
                  <div className="pt-6 border-t border-indigo-500/50">
                    {activeStudents.some(s => s.studentId === foundStudent.studentId) ? (
                      <button onClick={() => handleCheckOut(foundStudent.studentId)} className="w-full bg-white text-indigo-700 py-6 rounded-2xl font-black text-xl flex items-center justify-center gap-3 shadow-xl">
                        <LogOut size={22}/> 퇴실 처리
                      </button>
                    ) : (
                      <button onClick={handleCheckIn} className="w-full bg-emerald-400 text-white py-6 rounded-2xl font-black text-xl shadow-xl flex items-center justify-center gap-3">
